...
export class Conv2DProgramCS implements GPGPUProgram {
  ...
  constructor(convInfo: Conv2DInfo, outTexShape: string) {
    ...
    if (outTexShape === 'NHW_C') {
      this.localGroupSize = [8, 7];
      // outWidth must be divisible by localGroupSize[1]
      ...
      `;
    } else {  // outTexShape === '1H_WC'
      this.localGroupSize = [8, 16];
      // outChannels must be divisible by localGroupSize[0]
      ...
      this.userCode = `
        ...
        const int CACHE_H = ${
          (this.localGroupSize[1] - 1) * strideHeight + filterHeight +
          (filterHeight - 1) * (dilationHeight - 1)};
        const int CACHE_W = ${filterWidth};
        const int CACHE_C = ${convInfo.inChannels};
        const int CACHE_WC = CACHE_W * CACHE_C;
        const int CACHE_HWC = CACHE_H * CACHE_W * CACHE_C;
        shared float cache[CACHE_H][CACHE_W * CACHE_C];
        ...
        void main() {
          ...
          while (index < CACHE_HWC) {
            ...
            int xR = cacheRCorner + r;
            int xC = cacheCCorner + c * ${dilationWidth};
            if (xR >= 0 && xR < ${convInfo.inHeight} &&
                xC >= 0 && xC < ${convInfo.inWidth}) {
              cache[r][cd] = getX(batch, xR, xC, d);
            }
          ...
          // Discard threads that are out of Y bounds
          if (int(gl_GlobalInvocationID.y) >= ${convInfo.outHeight}) {
            return;
          }
